<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-0.5">Manage personal credentials, profile identity, and account
    ...
    <div class="space-y-6">
        ...
        <div
            class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="flex items-center gap-4">
                <!-- Large Avatar / Monogram -->
                <div class="relative">
                    <div
                        class="w-16 h-16 rounded-2xl bg-gradient-to-tr from-slate-900 to-slate-700 text-white flex items-center justify-center font-bold text-xl shadow-xs ring-4 ring-slate-100/80">
                        {{ strtoupper(substr($user->name, 0, 2)) }}
                    </div>
                    <span class="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-emerald-500 ring-2 ring-white"
                ...
                <div>
                    <div class="flex items-center gap-2.5 flex-wrap">
                        ...
                        @if($user->role->value === 'super_admin')
                            <span
                                class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-violet-50 text-violet-700 border border-violet-200/70 whitespace-nowrap">
                                <svg class="w-3 h-3 text-violet-500 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        @else
                            <span
                                class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-slate-100 text-slate-700 border border-slate-200 whitespace-nowrap">
                                Program Staff
                            </span>
                        @endif
                    </div>
            ...
            <div
                class="flex items-center gap-3 sm:gap-6 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6 text-xs">
                <div>
                    <span class="block text-[11px] font-medium text-slate-400 uppercase tracking-wider">Member
                        Since</span>
