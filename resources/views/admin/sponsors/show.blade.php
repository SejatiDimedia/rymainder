<x-app-layout>
    ...
    <div 
        x-data="{ 
            ...
            openRetryModal(url, channel, wave, due, error) {
                this.retryUrl = url;
        ...
        @keydown.escape.window="if (!isSubmittingRetry) { reminderModalOpen = false; retryModalOpen = false; }"
        class="space-y-6"
    >
                                    ...
                                    <td class="px-5 py-3 text-right whitespace-nowrap">
                                        @if($log->status->value === 'failed')
                                            <button 
                                                ...
                                                data-wave="{{ $log->reminderSetting->label ?? ($log->customReminder->title ?? ($log->is_manual ? 'Manual Direct' : 'Custom Direct Reminder')) }}"
                                                data-due="{{ $log->due_date->translatedFormat('d M Y') }}"
                                                data-error="{{ $log->error_message ?? '' }}"
                                                @click="openRetryModal($el.dataset.url, $el.dataset.channel, $el.dataset.wave, $el.dataset.due, $el.dataset.error)"
        ...
        >
            ...
            <div 
                ...
                @click="if(!isSubmittingRetry) retryModalOpen = false"
            ></div>
                ...
                >
                    ...
                    <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-sky-50/40">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 class="text-base font-bold text-slate-900" id="retry-modal-title">Confirm Reminder Resend</h3>
                                <p class="text-xs text-slate-500 mt-0.5">Resend notification to {{ $sponsor->name }}</p>
                            </div>
                        </div>
                        <button 
                            type="button" 
                            @click="retryModalOpen = false" 
                    ...
                    <div class="p-6 space-y-4">
                        <div class="p-4 bg-slate-50 rounded-xl border border-slate-200/80 space-y-2 text-xs">
                            <div class="flex items-center justify-between">
                                <span class="text-slate-500">Recipient Sponsor:</span>
                                <span class="font-bold text-slate-900">{{ $sponsor->name }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-slate-500">Notification Channel:</span>
                                <span class="font-semibold text-sky-700" x-text="retryChannel"></span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-slate-500">Reminder Schedule:</span>
                                <span class="font-medium text-slate-800" x-text="retryWave"></span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-slate-500">Target Due Date:</span>
                                <span class="font-medium text-slate-700" x-text="retryDue"></span>
                            </div>
                        </div>

                        <!-- Previous Error Callout -->
                        <div x-show="retryError" class="p-3 bg-rose-50 border border-rose-100 rounded-xl text-[11px] text-rose-700">
                            <p class="font-semibold text-rose-800 mb-0.5">Previous Delivery Error:</p>
                            <p class="font-mono break-words leading-relaxed" x-text="retryError"></p>
                        </div>

                        <p class="text-[11px] text-slate-400 leading-normal">
                            This reminder will be placed into the queue for immediate delivery through the provider.
                        </p>
                    </div>

                    <!-- Footer -->
                    <form method="POST" :action="retryUrl" @submit="isSubmittingRetry = true">
                        @csrf
                        <div class="px-6 py-4 bg-slate-50/80 border-t border-slate-100 flex items-center justify-end gap-3">
                            ...
                            >
                                <svg x-show="isSubmittingRetry" class="animate-spin -ml-0.5 w-3.5 h-3.5 text-white" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                <svg x-show="!isSubmittingRetry" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/></svg>
                                <span x-text="isSubmittingRetry ? 'Sending...' : 'Confirm Resend'"></span>
                            </button>
                        </div>
                    </form>
                </div>
