<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-xl font-bold text-slate-900 tracking-tight">Log Audit Pengiriman Reminder</h1>
                <p class="text-xs text-slate-500 mt-0.5">Audit trail terpusat seluruh pengiriman notifikasi via Email, WhatsApp, dan Telegram</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
            <form method="GET" action="{{ route('admin.logs.index') }}" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
                <!-- Search Input -->
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1.5">Cari Sponsor</label>
                    <input 
                        type="text" 
                        name="search" 
                ...
                <!-- Status Filter -->
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1.5">Status Pengiriman</label>
                    <select 
                        name="status" 
                ...
                <!-- Channel Filter -->
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1.5">Channel Notifikasi</label>
                    <select 
                        name="channel" 
                        class="w-full text-xs rounded-xl border-slate-200 bg-slate-50/50 text-slate-900 focus:border-slate-400 focus:ring-slate-400"
                    >
                ...
                <!-- Date Filter -->
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1.5">Target Jatuh Tempo</label>
                    <input 
                        type="date" 
                        name="date" 
                        value="{{ request('date') }}" 
                ...
                <div class="flex items-end gap-2">
                    <button 
                        type="submit" 
                        class="w-full px-3 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-semibold shadow-xs transition"
                    >
                        Filter
                    </button>
                    @if(request()->hasAny(['search', 'status', 'channel', 'date']))
                        <a 
                            ...
                            class="px-3 py-2.5 border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-xl text-xs font-medium transition"
                        >
                            Reset
                        </a>
                    @endif
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-6 space-y-4">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="font-bold text-slate-900 text-sm">Riwayat Aktivitas Pengiriman</h2>
                    <p class="text-xs text-slate-500 mt-0.5">Seluruh rekaman eksekusi queue worker dengan pesan provider</p>
                </div>
                <div class="text-xs text-slate-500">
                    Total: <span class="font-semibold text-slate-800">{{ $logs->total() }}</span> log
                </div>
            </div>
                    ...
                    <table class="w-full text-left text-xs">
                        <thead class="bg-slate-50/70 border-b border-slate-200/70 text-[11px] font-semibold uppercase tracking-wider text-slate-400">
                            <tr>
                                ...
                                <th class="px-5 py-3.5">Sponsor & Kontak</th>
                                <th class="px-5 py-3.5">Gelombang / Wave</th>
                                <th class="px-5 py-3.5">Channel</th>
                                <th class="px-5 py-3.5">Target Tempo</th>
                                <th class="px-5 py-3.5">Status</th>
                                <th class="px-5 py-3.5">Detail Respon</th>
